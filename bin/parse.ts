...
let outcome: Data[] | undefined,
  tmp: Data[] = [],
  RE_LINE =
    /^((\d{4})-(\d{2})-(\d{2})),"([^"]+)","([^"]+)","\s*(\d+\.\d+)","([^"]+)?","([^"]+)?","([^"]+)?","([^"]+)"$/,
  total = 0,
  category: Record<string, number> = {},
  subcategory: Record<string, Record<string, number>> = {},
  months: Record<string, number> = {},
  monthsCategory: Record<string, Record<string, number>> = {};
...
var source = fs
  .readFileSync(__dirname + "/../data/Report.csv", "utf-8")
  .split(/\n/)
...
(function () {
  ...
  Object.keys(category)
    ...
    .forEach(function (key) {
      ...
      Object.keys(subcategory[key])
        .sort(function (a, b) {
          return subcategory[key][a] == subcategory[key][b]
            ? 0
            : subcategory[key][a] > subcategory[key][b]
              ? -1
              : 1;
        })
        .map(function (subkey) {
...
function printMonths(title: string, data: Record<string, number>) {
  console.log("\t" + title);
