describe('Step 0: Jest Installation', () => {
  test('jest is installed and configured', () => {
    const fs = require('fs');
    const packageJson = JSON.parse(fs.readFileSync('/Users/latrekc/Code/budget/package.json', 'utf-8'));
  ...
  test('jest config file exists', () => {
    const fs = require('fs');
    expect(fs.existsSync('/Users/latrekc/Code/budget/jest.config.js')).toBe(true);
  });

  test('jest setup file exists', () => {
    const fs = require('fs');
    expect(fs.existsSync('/Users/latrekc/Code/budget/jest.setup.js')).toBe(true);
  });
